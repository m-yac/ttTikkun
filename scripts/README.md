...
Collect the ULT first, then build:
...
`en_ult_clean/`, which `combine` reads instead of the submodule. Rerun it
whenever the submodule updates: books upstream has fixed advance on their own,
and `en_ult_clean/README.md` records where each file came from.
...
Note that this rebuilds `src/data`, exactly as `combine` would: the build is
...
```
$ poetry run validate-ult ../en_ult/04-NUM.usfm ../hbo_uhb/04-NUM.usfm
18:23 occurrence-out-of-bounds: 'וְ⁠עָבַ֨ד' is occurrence 2 of only 1
```

Both are thin wrappers over `combine/usfm.py`, which parses a ULT book against
